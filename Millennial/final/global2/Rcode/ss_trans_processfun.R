...
calpars_SSfun <- function(sstype,fitpars){
  ...
  SStime = 365*loop #最多loop100年,100是不是太多？
...
##求Ts、NPP当前时期的平均值
mutliY_mean <- function(df0){
  if (ncol(df0) %% 12 == 0) {
    cols_per_year <- 12
  } else if (ncol(df0) %% 365 == 0) {
    cols_per_year <- 365
  } else {
    stop("The length of forcing data is wrong!")
  }
  
  num_years <- ncol(df0) / cols_per_year ## compute the number of year
  yearly_means <- matrix(NA, nrow = nrow(df0), ncol = num_years)
  
  # The average of each year
  for (i in 1:num_years) {
    start_col <- (i - 1) * cols_per_year + 1
    end_col <- i * cols_per_year
    yearly_means[, i] <- rowMeans(df0[, start_col:end_col, drop = FALSE], na.rm = TRUE)
  }
  
  multi_year_mean <- rowMeans(yearly_means, na.rm = TRUE) # Multi-year Average
  if(any(class(df0)=="data.frame")){
    multi_year_mean = as.data.frame(multi_year_mean)
  }
