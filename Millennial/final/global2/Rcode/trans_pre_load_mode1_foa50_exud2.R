...
for(i in c(2,3,5)[1:2]){#ssp为3:5
  ...
  TsNPPnm = c("","Ts","NPP_Oa","NPP_Exud","NPP_OaAExud")[1] #调整驱动数据是基准还是正常的
...
#  outfnm = paste0(outfnm, "_",testoutnm)

  sink(file.path(Rgl,"outTXT",paste0(outfnm,chagnm,TsNPPnm,sitenml,"_",fexu_oa,"_",fexudmod,"_output.txt")))
  cat("outfnm=",outfnm,"interval=",interval,"outflux=",outflux,"clab=",clab,"\n")
  ...
  if(scase=="ss"){
    ...
    print(forc_npp_all[20,][1:6])

    nloop = 0
    outname = c("year",fixCp,"Tco2",allflux.name,fixCp_c13,"Tco2_c13")
    derivnm = paste0(rep(outname,npar),".",rep(1:npar,each=length(outname)))
    outdir =  file.path(outdir0,scase)
    
    if(scase=="hist"){
      nyear = 25
      inout = file.path(outdir0,"ss")
#      load(file.path(inout,paste0(modelnm,"_fexu_oa-",fexu_oa,"_",fexudmod,"_ss",chagnm,sitenml,".Rdata"))) #initial c pools
      load(file.path(inout,paste0(modelnm,"_fexu_oa-",fexu_oa,"_",fexudmod,"_ss","_0001_7527",chagnm,".Rdata"))) #test
    }else{
      nyear = 86
      inout = file.path(outdir0,"hist")
      load(file.path(inout,paste0(modelnm,"_hist",chagnm,"_",TsNPPnm,sitenml,"_fexu_oa-",fexu_oa,"_",fexudmod,"_outdf.Rdata"))) #initial c pools
    }
    incpools = outdf
    sstime = 365* nyear
    cat("rows of soilPsim:",nrow(soilPsim),"forc_st=",nrow(forc_st_all),'\n')
    source(file.path(Rgl,"derivs_run_benf.R"))
