...
struct StrsoilC{
  double LIT, SOM, POM, LMWC, MIC, MAOM, MA, MD, Tco2;
};
struct Strflux{
...
//Trace CO2 from organic acid result in desorption (without loop)
void CalLabC(StrsoilC& c13, StrsoilC c0, StrsoilC c1, Strflux flux,float CUE){  
  // 标记只来自：有机酸解吸附； 只要离开LWMC，标记都会消失（LM->MAOC, LM->MB, leaching 三条路径)
  Strflux lab;
  ...
  float sum_tmp = lab.f_LM_leach + lab.f_LM_MA;
  if(sum_tmp > c13.LMWC){
    lab.f_LM_leach = c13.LMWC * (lab.f_LM_leach/sum_tmp);
    lab.f_LM_MA = c13.LMWC * (lab.f_LM_MA/sum_tmp);
  }
  
  // Update C pools
  c13.POM = 0; 
  c13.LMWC = c13.LMWC - lab.f_LM_leach - lab.f_LM_MA;
  c13.MA = 0; 
  c13.MD = 0; 
  c13.MAOM = 0;
  c13.MIC = 0;
  c13.SOM = 0;
  c13.Tco2 = c13.Tco2 + lab.f_MA_co2;
}


//Trace CO2 from old carbon, that is MAOC
void CalLabOldC(StrsoilC& oldc, StrsoilC c0, StrsoilC c1, Strflux flux,float CUE){  
  // 标记只来自MAOC：有机酸解吸附,米氏方程，其他解吸附； 只要离开LWMC，标记都会消失（LM->MAOC, LM->MB, leaching 三条路径)
  Strflux lab;
  
  // input
  oldc.LMWC =  oldc.LMWC + flux.acid_des + flux.f_MA_LMe + flux.f_MA_LMm;
  
  // LM->MB
  double LMWC = (c1.LMWC + flux.f_LM_MB);
  if(LMWC > 0. && oldc.LMWC > 0.){
    lab.f_LM_MB = fmin(flux.f_LM_MB * oldc.LMWC/LMWC, oldc.LMWC);
    lab.f_MA_co2 = lab.f_LM_MB * (1. - CUE); //flux.f_MA_co2  * c13.LMWC/LMWC;
  }else{
    lab.f_LM_MB = 0.; 
    lab.f_MA_co2 = 0. ;
  }
  oldc.LMWC = oldc.LMWC - lab.f_LM_MB;
  
  // LM->MAOC, leaching
  if(c1.LMWC > 0. && oldc.LMWC >0.){
    lab.f_LM_MA =  fmin(flux.f_LM_MA  * oldc.LMWC/c1.LMWC, oldc.LMWC);
    lab.f_LM_leach =  fmin(flux.f_LM_leach  * oldc.LMWC/c1.LMWC, oldc.LMWC);
  }else{
    lab.f_LM_MA = 0.;
    lab.f_LM_leach = 0.; 
  }
  ...
  float sum_tmp = lab.f_LM_leach + lab.f_LM_MA ;
  if(sum_tmp > oldc.LMWC){
    lab.f_LM_leach = oldc.LMWC * (lab.f_LM_leach/sum_tmp);
    lab.f_LM_MA = oldc.LMWC * (lab.f_LM_MA/sum_tmp);
  }
  
  // Update C pools
  oldc.POM = 0; 
  oldc.LMWC = oldc.LMWC - lab.f_LM_leach - lab.f_LM_MA;
  oldc.MA = 0; 
  oldc.MD = 0; 
  oldc.MAOM = 0;
  oldc.MIC = 0;
  oldc.SOM = 0;
  oldc.Tco2 = oldc.Tco2 + lab.f_MA_co2;
}


//Trace CO2 from plant input C directly to LMWC, 2025.09.02
void CalLabCInputCtoLMWC(StrsoilC& pi2lmwc,StrsoilC c0, StrsoilC c1, Strflux flux,float CUE){
  
  Strflux lab;
  // C input
  //  pi2lmwc.LMWC =  pi2lmwc.LMWC + (flux.f_Lit_LM + flux.f_Exud_LM + flux.f_Oa_LM);
  
  pi2lmwc.LMWC =  pi2lmwc.LMWC + flux.f_Oa_LM;
  
  double LMWC = (c1.LMWC + flux.f_LM_MB);
  if(LMWC > 0. && pi2lmwc.LMWC > 0.){
    lab.f_LM_MB = fmin(flux.f_LM_MB * pi2lmwc.LMWC/LMWC, pi2lmwc.LMWC);
    lab.f_MA_co2 = lab.f_LM_MB * (1. - CUE); //flux.f_MA_co2  * pi2lmwc.LMWC/LMWC;
    
  }else{
    lab.f_LM_MB = 0.; 
    lab.f_MA_co2 = 0. ;
  }
  pi2lmwc.LMWC = pi2lmwc.LMWC - lab.f_LM_MB;
  
  
  if(c1.LMWC > 0. && pi2lmwc.LMWC >0.){
    lab.f_LM_MA =  fmin(flux.f_LM_MA  * pi2lmwc.LMWC/c1.LMWC, pi2lmwc.LMWC);
    lab.f_LM_leach =  fmin(flux.f_LM_leach  * pi2lmwc.LMWC/c1.LMWC, pi2lmwc.LMWC);
  }else{
    lab.f_LM_MA = 0.;
    lab.f_LM_leach = 0.; 
  }
  
  //if((lab.f_LM_leach + lab.f_LM_MA)>pi2lmwc.LMWC){
  //  lab.f_LM_leach = lab.f_LM_MA = pi2lmwc.LMWC/2;
  //}
  
  float sum_tmp = lab.f_LM_leach + lab.f_LM_MA ;
  if(sum_tmp > pi2lmwc.LMWC){
    lab.f_LM_leach = pi2lmwc.LMWC * (lab.f_LM_leach/sum_tmp);
    lab.f_LM_MA = pi2lmwc.LMWC * (lab.f_LM_MA/sum_tmp);
  }
  
  //  new modified, 2025.09.02
  float dLMWC =  flux.f_Lit_LM + flux.f_Exud_LM - (lab.f_LM_leach + lab.f_LM_MA);
  
  // Update C pools
  pi2lmwc.POM = 0; //pi2lmwc.POM  + dPOM;
  pi2lmwc.LMWC = pi2lmwc.LMWC + dLMWC;
  pi2lmwc.MA = 0; //pi2lmwc.MA + dMA;
  pi2lmwc.MD = 0; //pi2lmwc.MD + dMD;
  pi2lmwc.MAOM = 0; //pi2lmwc.MAOM + dMAOM;
  pi2lmwc.MIC = 0; //pi2lmwc.MA + pi2lmwc.MD;
  pi2lmwc.SOM = 0;
  pi2lmwc.Tco2 = pi2lmwc.Tco2 + lab.f_MA_co2;
  return;
...
vector<vector<double>> derivs_V2_MM_AD_CO2(int sstime, List parsSoiList, List CpoolsList,NumericVector forc_st,
                                           ...
                                           NumericVector forc_exud, 
                                           bool outflux, int nloop,bool clab,List C13poolsList,bool OaChangDes0,
                                           List oldcpoolsList, List pi2lmwcpoolsList) {
  //output
  int ncpools = 7, nTco2 = 1, curtime = 0, nyear,nyear_row, nrep,nflux=0;//SOM,Cpools,Tco2
  if(nloop==0 || nloop>1000){//1constant dynamic input,test,100
    nyear = sstime/365;
    nrep = 365;
  }else{
...
//  NumericVector forc_exud = forc_acid; // root exudates exclude organic acid
  ...
  //label old carbon from MAOC flux; label plant c input to lmwc
  StrsoilC c13, oldc , pi2lmwc;
  StrsoilC c0, c1;
  // cout << "forc_sw.size()="<<forc_sw.size()<<endl;
 
  for(int t=0; t<sstime; ++t){
    c0 = {0, POM, LMWC, MIC, MAOM, MA, MD, 0};
    
    int fdi = t%forc_sw.size();
...
//    float vmax_pl = pars.Vpl0 * tp_scalar("POM", forc_st[fdi], Tref) * pow(forc_sw0 / pars.porosity, 1);
    ...
    if( MAOM > 0.f ){
      ...
      acid_des = fmax((pow(MAOM/unit_kgTom2 *1e-3,0.04f) *pow(forc_acid[oati]/unit_kgTom2, 0.87f)
                    * pow(pars.param_clay,-0.24f) * pow(pars.param_pH,-2.15f) * 34) * unit_kgTom2,0.f);
      acid_des = fmin(acid_des,MAOM);
    }else{
      acid_des = 0.f;
    }

    if (param_qmax < (MAOM  + f_Oa_MA)) {
      f_Oa_LM = f_Oa_LM + (f_Oa_MA - (param_qmax - MAOM));
      f_Oa_MA = param_qmax - MAOM;
    }
...
//         break;
//      }else{
         //cout << "Primary CUE = " << CUE;
         CUE = fmax(fmin(CUE,CUE_max), CUE_min);
         //cout << "Refresh CUE = " << CUE << "t = " << t << "fdi = " << fdi << endl;
//      }
    }
...
//    float f_MB_MA = f_MB_turn * pars.param_pb;
    ...
    // flux balance
    if (param_qmax < (MAOM  + f_LM_MA + f_MB_MA + f_PO_MA)) {
      if(f_PO_MA == 0){
        float tinmaoc = (param_qmax - param_qmax)/2;
        f_LM_MA = tinmaoc;
        f_MB_MA = tinmaoc;
      }else{
        float tinmaoc = (param_qmax - param_qmax)/3;
        f_LM_MA = tinmaoc;
        f_MB_MA = tinmaoc;
        f_PO_MA = tinmaoc;
      }
    ...
    if(ti==0){
      ...
      ayear[ti][7] = ayear[ti-1][7]+ Tco2;
    }
    if(outflux){
      if(ti==0){
        ayear[ti][8] = f_Lit_PO, ayear[ti][9] = f_Lit_LM;
        ayear[ti][10] = f_Exud_LM, ayear[ti][11] = f_Oa_MA;
        ayear[ti][12] = f_Oa_LM, ayear[ti][13] = f_PO_LM;
        ayear[ti][14] = f_PO_MA, ayear[ti][15] = f_MA_LMm;
        ayear[ti][16] = f_MA_LMe, ayear[ti][17] = acid_des;
        ayear[ti][18] = f_MA_LM, ayear[ti][19] = f_LM_MA;
        ayear[ti][20] = f_MA_MD, ayear[ti][21] = f_MD_MA;
        ayear[ti][22] = f_LM_MB, ayear[ti][23] = f_MA_maint_co2;
        ayear[ti][24] = f_MA_growth_co2, ayear[ti][25] = f_MA_co2;
        ayear[ti][26] = f_MD_co2, ayear[ti][27] = f_MA_growth;
        ayear[ti][28] = f_MB_LM, ayear[ti][29] = f_MB_MA;
        ayear[ti][30] = f_MB_turn, ayear[ti][31] = f_LM_leach;
      }else{
        ayear[ti][8] = ayear[ti-1][8]+ f_Lit_PO;
        ayear[ti][9] = ayear[ti-1][9]+ f_Lit_LM;
        ayear[ti][10] = ayear[ti-1][10]+ f_Exud_LM;
        ayear[ti][11] = ayear[ti-1][11]+ f_Oa_MA;
        ayear[ti][12] = ayear[ti-1][12]+ f_Oa_LM;
        ayear[ti][13] = ayear[ti-1][13]+ f_PO_LM;
        ayear[ti][14] = ayear[ti-1][14]+ f_PO_MA;
        ayear[ti][15] = ayear[ti-1][15]+ f_MA_LMm;
        ayear[ti][16] = ayear[ti-1][16]+ f_MA_LMe;
        ayear[ti][17] = ayear[ti-1][17]+ acid_des;
        ayear[ti][18] = ayear[ti-1][18]+ f_MA_LM;
        ayear[ti][19] = ayear[ti-1][19]+ f_LM_MA;
        ayear[ti][20] = ayear[ti-1][20]+ f_MA_MD;
        ayear[ti][21] = ayear[ti-1][21]+ f_MD_MA;
        ayear[ti][22] = ayear[ti-1][22]+ f_LM_MB;
        ayear[ti][23] = ayear[ti-1][23]+ f_MA_maint_co2;
        ayear[ti][24] = ayear[ti-1][24]+ f_MA_growth_co2;
        ayear[ti][25] = ayear[ti-1][25]+ f_MA_co2;
        ayear[ti][26] = ayear[ti-1][26]+ f_MD_co2;
        ayear[ti][27] = ayear[ti-1][27]+ f_MA_growth;
        ayear[ti][28] = ayear[ti-1][28]+ f_MB_LM;
        ayear[ti][29] = ayear[ti-1][29]+ f_MB_MA;
        ayear[ti][30] = ayear[ti-1][30]+ f_MB_turn;
        ayear[ti][31] = ayear[ti-1][31]+ f_LM_leach;
      }
    }
    curtime = curtime + 1;
    
    if(clab ){ //&& nrep != 365
      c1 = {0, POM, LMWC, MIC, MAOM, MA, MD, 0};
      Strflux flux0 = {f_Lit_PO, f_Lit_LM, f_Exud_LM, f_Oa_MA, f_Oa_LM,
                       ...
                       f_PO, f_MB_PO //new add
      };
        
 
      if(t==0){
        c13.POM = as<double>(C13poolsList["POM"]); 
        ...
        // label old carbon
        oldc.POM = as<double>(oldcpoolsList["POM"]); 
        oldc.LMWC = as<double>(oldcpoolsList["LMWC"]); 
        oldc.MIC = as<double>(oldcpoolsList["MIC"]); 
        oldc.MAOM = as<double>(oldcpoolsList["MAOM"]); 
        oldc.MA = as<double>(oldcpoolsList["MA"]); 
        oldc.MD = as<double>(oldcpoolsList["MD"]); 
        oldc.Tco2 = 0.; 
        oldc.SOM = oldc.POM + oldc.LMWC + oldc.MIC + oldc.MAOM;
        
                // label old carbon
        pi2lmwc.POM = 0; //as<double>(pi2lmwcpoolsList["POM"]); 
        pi2lmwc.LMWC = as<double>(pi2lmwcpoolsList["LMWC"]); 
        pi2lmwc.MIC = 0; //as<double>(pi2lmwcpoolsList["MIC"]); 
        pi2lmwc.MAOM = 0; //as<double>(pi2lmwcpoolsList["MAOM"]); 
        pi2lmwc.MA = 0; //as<double>(pi2lmwcpoolsList["MA"]); 
        pi2lmwc.MD = 0; //as<double>(pi2lmwcpoolsList["MD"]); 
        pi2lmwc.Tco2 = 0.; 
        pi2lmwc.SOM = pi2lmwc.POM + pi2lmwc.LMWC + pi2lmwc.MIC + pi2lmwc.MAOM;
      }

      CalLabC(c13, c0, c1, flux0,CUE);
        // label old carbon
      CalLabOldC(oldc, c0, c1, flux0,CUE);
        // label plant C directly to LMWC
      CalLabCInputCtoLMWC(pi2lmwc, c0, c1, flux0, CUE);
      ...
      ayear[ti][colth+1] = c13.POM;
      ayear[ti][colth+2] = c13.LMWC;
      ayear[ti][colth+3] = c13.MIC;
      ayear[ti][colth+4] = c13.MAOM;
      ayear[ti][colth+5] = c13.MA;
      ayear[ti][colth+6] = c13.MD;
      ayear[ti][colth+7] = c13.Tco2;

              // label old carbon
      ayear[ti][colth+8] = oldc.SOM;
      ayear[ti][colth+9] = oldc.POM;
      ayear[ti][colth+10] = oldc.LMWC;
      ayear[ti][colth+11] = oldc.MIC;
      ayear[ti][colth+12] = oldc.MAOM;
      ayear[ti][colth+13] = oldc.MA;
      ayear[ti][colth+14] = oldc.MD;
      ayear[ti][colth+15] = oldc.Tco2;

                    // plant C directly to LMWC
      ayear[ti][colth+16] = pi2lmwc.SOM;
      ayear[ti][colth+17] = pi2lmwc.POM;
      ayear[ti][colth+18] = pi2lmwc.LMWC;
      ayear[ti][colth+19] = pi2lmwc.MIC;
      ayear[ti][colth+20] = pi2lmwc.MAOM;
      ayear[ti][colth+21] = pi2lmwc.MA;
      ayear[ti][colth+22] = pi2lmwc.MD;
      ayear[ti][colth+23] = pi2lmwc.Tco2;
    }
    ...
    if(curtime%365 == 0){
      ...
      c13.Tco2 = 0.;
      oldc.Tco2 = 0.;// label old carbon
      pi2lmwc.Tco2 = 0.;// label plant c to lmwc
      ...
      if(nloop==0){
        ...
        for (int ci = 0; ci < (ayear[0].size() -1 ); ++ci) {
          float sum = 0.f;
          if(ci==7 || ci==15 || ci==23){ //Tco2 exclude
            continue;
          }
